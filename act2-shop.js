...
restarts the tier LABEL at 1 while its actual bonus value (+6 to a
primary stat) is meaningfully stronger than Gladstone's own Tier 4
(+4) — this whole ladder continues climbing from there (+6 -> +8 ->
+10 -> +12) rather than mirroring Act 1's +1/+2/+3/+4 spacing.

That +6 isn't an arbitrary "stronger than Act 1" number — it's picked
so Tier 1's own level requirement (getGearRequirements(), item-tiers.js
— 2x an item's PRIMARY stat, deliberately no longer summing secondary
stats into it) lands at exactly level 12, matching Mudroot Warren's own
ZONE_LEVEL_RECOMMENDATION floor (content.js) after that got re-tuned
from real playtest data. +8/+10/+12 continue that same "level req =
primary x2" read climbing 16/20/24 — 16 lines up with Mudroot Warren's
own max/Warren's Ear's own min, 20/24 sit inside and past Warren's Ear's
own open-ended range. An earlier pass here used +8/+10/+12/+16 (chosen
to literally double Act 1 Tier 4's own +4 at every step) before the
zone-level re-tune above made clear Tier 1's resulting level-24
requirement was badly out of step with when a player actually reaches
Mudroot Warren.

Secondary stats scale WITH the primary here too, same as every Act 1
tier — rollShopGearStats() (economy.js), which rerolls a purchase's
secondary stat(s) at buy time, rolls each one from half the primary's
value up to the full value (rollSecondaryStatValue(), economy.js), so
there was no reason to invent a different secondary convention here.
Secondary stats don't factor into the level requirement at all
anymore (see item-tiers.js's own comment) — only the primary does.
